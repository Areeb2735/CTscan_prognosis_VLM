...
class CTCLIP(nn.Module):
    def __init__(
        ...
        # self.final_linear_layer_1 = nn.Linear(dim_latent, 1024, bias = False)
        # self.final_linear_layer_2 = nn.Linear(1024, dim_latent, bias = False)
    ...
    def forward(
        ...
        with torch.no_grad():
            # enc_image= self.visual_transformer(image, pt_scan, return_only_spatial=True, return_encoded_tokens=True)
            enc_image= self.visual_transformer(image, return_only_spatial=True, return_encoded_tokens=True)
       ...
       # print(enc_image.shape, flush=True)
        ...
        # print("text embd", text_embeds.shape, text_embeds.max(), text_embeds.min())
        # print("image embd", image_embeds.shape, image_embeds.max(), image_embeds.min())
        ...
        # print("text latents", text_latents.shape, text_latents.max(), text_latents.min())
        # print("image latents", image_latents.shape, image_latents.max(), image_latents.min())
        
        # print("text latents shape", text_latents.shape)
        # print("image latents shape", image_latents.shape)
        if prognosis:
            combined_embeddings = text_latents + image_latents
            return combined_embeddings
            # final_output = self.final_linear_layer(combined_embeddings)
        if embed:
